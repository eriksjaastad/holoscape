...
6. THE Holoscape SHALL NOT delete `ShapedWindowController.buildMaskLayer`, `WindowDragOverlay`, polygon scaling, `windowDidResizeForShape`, or `writeShapeDiagnostic` until every in-tree shaped skin (HoloscapeSynthwave, HoloscapeClassic, AmplifyDemo) has migrated to v4 chrome with animations verified live and every `BackwardCompatIntegrationTests` scenario passes.
