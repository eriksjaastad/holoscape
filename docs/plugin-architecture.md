...
- `Sources/Holoscape/Services/PluginManager.swift` — side-effect-free startup coordinator that validates bundled manifests, prepares enabled first-party runtimes, publishes optional plugin contribution descriptors, removes all contributions for disabled plugins, and records manifest/config/runtime preparation failures as plugin-scoped diagnostics instead of throwing through core startup.
