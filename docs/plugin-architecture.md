...
- `Sources/Holoscape/Services/ProjectTrackerPlugin.swift` — bundled first-party Project Tracker plugin descriptor, side-effect-free start-plan gate, scoped runtime health probe, explicit Project Tracker board URL builder, plugin-local command action resolver, and supplemental status-adapter snapshot. It declares optional channel, command, and status-adapter capabilities plus only `network:localhost` and plugin-storage permissions; non-local endpoints require an explicit `network:host:<hostname>` manifest permission before any client can start. The health probe consumes an already-validated runtime plan and reports available/unavailable as plugin-local state instead of changing core channel state. The status-adapter snapshot is supplemental UI metadata, not a replacement for core persistent tab state. The command resolver currently accepts only the namespaced `project-tracker-open-board` descriptor with an explicit `projectSlug` argument and returns an open-URL action; unknown commands and missing arguments fail instead of falling back to broader Project Tracker behavior.
