...
- `Sources/Holoscape/Services/ProjectTrackerPlugin.swift` — bundled first-party Project Tracker plugin descriptor, side-effect-free start-plan gate, scoped runtime health probe, explicit Project Tracker board/task URL builders, plugin-local command action resolver, and supplemental status-adapter snapshots. It declares optional channel, command, and status-adapter capabilities plus only `network:localhost` and plugin-storage permissions; non-local endpoints require an explicit `network:host:<hostname>` manifest permission before any client can start. The health probe consumes an already-validated runtime plan and reports available/unavailable as plugin-local state instead of changing core channel state. The task-status fetcher calls only the validated plugin endpoint's `/api/tasks?project_id=<slug>&include_archived=true` path from explicit runtime use, rejects invalid project slugs/task ids before network access, filters the returned task list by display id or canonical id, and surfaces missing-task/HTTP/transport/decoding failures as plugin-scoped unavailable metadata. The status-adapter snapshots are supplemental UI metadata, not replacements for core persistent tab state. The command resolver currently accepts only namespaced open-board/open-task descriptors with explicit `projectSlug` and `taskID` arguments and returns open-URL actions; unknown commands, invalid ids, and missing arguments fail instead of falling back to broader Project Tracker behavior.
- `Sources/Holoscape/Services/PluginStorageService.swift` — plugin-owned storage locator rooted under `plugins/<storageNamespace>/` inside the Holoscape config directory. It requires the already-validated runtime plan to declare `filesystem:plugin-storage`, rechecks namespace shape before path construction, creates only the plugin namespace on request, and never touches core stores such as sessions or scrollback.
- `Sources/Holoscape/Services/PluginCommandRouter.swift` — command execution seam for already-advertised plugin commands. It refuses commands absent from the active contribution snapshot before runtime resolution, requires a ready Project Tracker plan, and opens external URLs only through an injected opener so tests prove disabled/unadvertised plugin commands cannot perform side effects.
- `Sources/Holoscape/Models/HoloscapeConfig.swift` — optional `plugins.projectTracker` settings. Missing settings default to the local first-party plugin plan for backward compatibility; disabling the plugin short-circuits validation of endpoint/health-path values so stale plugin config cannot block standalone terminal startup.
- `Sources/Holoscape/Services/PluginManager.swift` — side-effect-free startup coordinator that validates bundled manifests, reads optional Project Tracker plugin settings from `HoloscapeConfig`, prepares enabled first-party runtimes, publishes optional plugin contribution descriptors with explicit required command arguments, removes all contributions for disabled plugins, and records manifest/config/runtime preparation failures as plugin-scoped diagnostics instead of throwing through core startup.
- `Sources/Holoscape/Services/SetupDiagnosticsService.swift` — visible setup diagnostics surface that includes plugin startup health from the side-effect-free `PluginManager`; Project Tracker plugin misconfiguration is shown as a plugin failure while terminal startup continues without plugin contributions.
- `Tests/HoloscapeTests/Unit/PluginManifestTests.swift` — regression coverage that core can run with zero plugins, rejects unknown manifest fields before start, prevents core storage namespace collisions, validates the bundled Project Tracker descriptor, rejects invalid or undeclared Project Tracker endpoints without falling back to localhost, validates health-path configuration before runtime start, keeps health failures plugin-scoped, builds Project Tracker board URLs without writing tracker ids into core state, exposes Project Tracker availability as supplemental plugin status, and verifies the PluginManager publishes/removes Project Tracker contributions only through the plugin seam.
- `Tests/HoloscapeTests/Unit/SetupDiagnosticsServiceTests.swift` — regression coverage that plugin preparation failures surface in Setup Diagnostics without blocking broker/core diagnostics and that healthy plugin startup reports a visible OK state.

This slice intentionally does not start a Project Tracker client during Holoscape startup, query PT from app startup, or make any plugin required for terminal/channel launch. It prepares explicit runtime and contribution descriptors that later UI code can consume after manifest validation and user configuration; only explicit plugin runtime calls perform health checks. Plugin settings live under an optional config namespace so old config files decode unchanged and core can keep launching even when the Project Tracker plugin is disabled or misconfigured.
